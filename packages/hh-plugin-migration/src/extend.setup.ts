...
export type ForkMigrationNetworkConfig = ReturnType<typeof loadMigrationForkNetworkConfig>;
export const loadMigrationForkNetworkConfig = (
    ...
    if (!hardhatNetworkUserConfig) {
        console.log(`no networks field present in hardhat.config.ts file, aborting.`);
        process.exit(-1);
    }
